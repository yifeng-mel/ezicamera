...
static GOptionEntry entries[] =
{
  ...
  { "file-name", 0, 0, G_OPTION_ARG_STRING, &file_name, "String file name to download", "File name" },
...
data_channel_on_open (GObject * dc, gpointer user_data)
{
  ...
  gchar *path = g_strdup_printf ("/videos/%s", file_name);
  gchar *file_path = g_filename_from_utf8(path, strlen(path), NULL, NULL, NULL);
  ...
  for(gsize i = 0; i < size; i = i + 16384 ){
    GBytes *segment_bytes = g_bytes_new_from_bytes (bytes, i, (i + 16384 >= size) ? (size - i) : (16384) );
    g_signal_emit_by_name (dc, "send-data", segment_bytes);
    g_bytes_unref (segment_bytes);
  }
...
start_pipeline (void)
{
  ...
  pipe1 =
      ...
      "queue ! " RTP_CAPS_OPUS "97 ! sendonly. ",
      &error);
